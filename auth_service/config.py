...
class Settings(BaseSettings):
    ...
    PROFILING_STATE: (
        Literal["disabled"] | Literal["suggesting"] | Literal["restricting"]
    ) = "suggesting"
    ...
    def validate_sql_url_fields(cls, url, values):
        if not parse.urlparse(url).scheme in {"http", "https", "mongodb"}:
            ...
            if isinstance(values["ID_COLUMN_TYPE"], "str") and values[
                "ID_COLUMN_TYPE"
            ] in {"int", "uuid"}:
                values["ID_COLUMN_TYPE"] = (
                    Integer if values["ID_COLUMN_TYPE"] == "int" else UUID
                )
        return url
